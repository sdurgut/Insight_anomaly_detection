...
def get_max_id(JsonData):
	...
	for x in JsonData:
		if x["event_type"] == "purchase":
			if int(x["id"]) > max_id: 
				max_id = int(x["id"])
		if x["event_type"] == "befriend" or x["event_type"] == "unfriend":
			if int(x["id1"]) > max_id: 
				max_id = int(x["id1"])
			if int(x["id2"]) > max_id: 
				max_id = int(x["id2"])
...
#
def update_purchaseHistory(amount, userPurchaseHistory, TRACKED_NUMBER):
	userPurchaseHistory.put(amount)
	if userPurchaseHistory.qsize() > TRACKED_NUMBER:
		userPurchaseHistory.get()
...
#
def get_mean(userPurchaseHistory):
	tmpList = list(userPurchaseHistory.queue)
	if len(tmpList) == 0: 
		return 0;
	sum = 0.0
	for x in tmpList:
...
#
def get_std_dev(userPurchaseHistory):
	tmpList = list(userPurchaseHistory.queue)
	if len(tmpList)==0:
		return 0;
	mean = get_mean(userPurchaseHistory)
	tmpSum = 0
	for x in tmpList:
...
#
def is_anomaly(amount, userPurchaseHistory):
	mean = get_mean(userPurchaseHistory)
	std_dev = get_std_dev(userPurchaseHistory)
...
# create list of purchase history as a queue
userPurchaseHistory = queue.Queue()


# loop over the lines of the log file, each line is a python dictionary
for dic in  batch_log:
	if dic["event_type"] == "purchase" :
		update_purchaseHistory(float(dic["amount"]),userPurchaseHistory,TRACKED_NUMBER)
...
for dic in stream_log:
	if dic["event_type"] == "purchase" :
		...
		if is_anomaly(float(dic["amount"]),userPurchaseHistory):
			mean = get_mean(userPurchaseHistory)
			std_dev = get_std_dev(userPurchaseHistory)
